...
const customFireEvent = <T extends keyof HASSDomEvents>(
  node: HTMLElement,
  type: T,
...
const getActionHandler = (): ActionHandler => {
  ...
  const existing = body.querySelector('.action-handler-last-seen-value-card');
  if (existing) {
    return existing as ActionHandler;
  }

  const div = document.createElement('div');
  div.className = 'action-handler-last-seen-value-card';
